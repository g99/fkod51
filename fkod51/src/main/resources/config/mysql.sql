...
SELECT DISTINCT MOVIE.FILM_NAME AS MOVIENAME 
		FROM THEATER, MOVIE 
		WHERE THEATER.THEATER_SEQ = (SELECT DISTINCT THEATER_SEQ FROM THEATER WHERE THEATER_NAME = '강남') AND MOVIE.FILM_NAME = (SELECT DISTINCT THEATER_SEQ FROM THEATER WHERE THEATER_NAME = '강남') AND SEAT_STATUS > 0 ORDER BY SCHEDULE.SHOW_DATE ASC

SELECT SEAT_NUMBER FROM SEAT2 WHERE SEAT_NUMBER LIKE '12-%';
select * from ticket;
SELECT SCHEDULE.SEQ FROM SCHEDULE
WHERE SCHEDULE.THEATER_SEQ = (SELECT THEATER_SEQ FROM THEATER WHERE THEATER_NAME = '강남') 
AND FILM_NUMBER = (SELECT FILM_NUMBER FROM MOVIE WHERE FILM_NAME = '검은 사제들') 
AND SCHEDULE.SHOW_DATE = '2015/11/18' 
AND SCHEDULE.SHOW_TIME = '12:40' 
AND SCHEDULE.ROOM_NAME = '3관';
